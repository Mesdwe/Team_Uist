...
inline float3 ComputeVectorCamToPixOSN(float3 pixPosOS, float3 cameraPosOS, half cameraIsOrtho)
{
    ...
    return lerp(vecCamToPixOSN, vecCamForwardOSN, cameraIsOrtho);
}
...
v2f vertShared(vlb_appdata v, float outsideBeam)
{
    ...
    // Vector Camera to current Pixel, in object space and normalized
    float3 vecCamToPixOSN = ComputeVectorCamToPixOSN(o.posObjectSpaceNonSkewed.xyz, cameraPosObjectSpace, cameraIsOrtho);
...
half4 fragShared(v2f i, float outsideBeam)
{
    ...
    // Vector Camera to current Pixel, in object space and normalized
    float3 vecCamToPixOSN = ComputeVectorCamToPixOSN(i.posObjectSpaceNonSkewed.xyz, cameraPosObjectSpace, cameraIsOrtho);
