Shader "Unlit/CurvedObject2"
{
    Properties {
        _MainTex ("Texture", 2D) = "white" {}
        _Color ("Diffuse Material Color", Color) = (1,1,1,1) 
        _Color2("Diffuse Material Color2", Color) = (1,1,1,1)
        _Boundary("Boundary", Range(-1, 1)) = 0
        _SpecColor ("Specular Material Color", Color) = (1,1,1,1) 
        _Shininess ("Shininess", Float) = 10
        _Intensity ("Intensity", Range(0.0,1.0)) = 0.5
        _Offset ("Offset",Vector) =(-1000,0,0)
        _Depth("Depth",float) = 300
    }
    SubShader {
        Pass {    
            ...
            float4 frag(vertexOutput input) : COLOR
            {
                ...
                float mask = saturate(input.posWorld.z/_Depth + _Boundary) / 2+0.5;

                float3 ambientLighting = 
                UNITY_LIGHTMODEL_AMBIENT.rgb * lerp(_Color.rgb, _Color2.rgb, mask);
                
                float3 diffuseReflection = 
                _LightColor0.rgb * lerp(_Color.rgb, _Color2.rgb, mask)
...
Pass {    
            ...
            float4 frag(vertexOutput input) : COLOR
            {
                ...
                float3 diffuseReflection = 
                attenuation * _LightColor0.rgb * lerp(_Color.rgb, _Color2.rgb, mask)
                * max(0.0, dot(normalDirection, lightDirection));
